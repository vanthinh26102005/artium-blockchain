    ...
    <div className="min-h-screen bg-white text-[#1a1c1c]" style={bodyFont}>
      <main className="mx-auto max-w-[1600px] px-6 pt-32 pb-20 md:px-12">
        <header className="mb-16 flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
          <div className="max-w-2xl">
            ...
            <p className="text-lg leading-8 font-light text-[#444747]">
              A curated selection of architectural masterworks and digital artifacts. Each piece is verified via blockchain provenance, ensuring institutional-grade collectible security.
            </p>
          </div>
        ...
        <section className="mb-12 flex flex-wrap items-center justify-between gap-8 border-y border-[#c4c7c7]/30 py-8">
          <div className="flex flex-wrap items-center gap-10">
            {filterGroups.map((group) => (
              <button key={group.label} type="button" className="group text-left">
                <span className="mb-2 block text-[10px] tracking-[0.25em] text-[#747777] uppercase">{group.label}</span>
                <span className="flex items-center gap-2 text-sm font-medium text-black transition-colors group-hover:text-black/70" style={headlineFont}>
                  {group.value}
          ...
          <div className="flex items-center gap-4">
            <div className="flex border border-[#c4c7c7]/30 bg-[#f7f7f7] p-1">
              <button type="button" aria-label="Grid view" className="bg-black px-3 py-2 text-white">
                <Grid2X2 className="h-4 w-4" />
              </button>
              <button type="button" aria-label="List view" className="px-3 py-2 text-black/40 transition-colors hover:text-black">
                <LayoutList className="h-4 w-4" />
              </button>
            </div>
            <button type="button" className="border border-black px-6 py-2 text-xs tracking-[0.22em] text-black uppercase transition hover:bg-black hover:text-white" style={headlineFont}>
              Filter Results
            </button>
          </div>
        </section>

        <section className="grid grid-cols-1 gap-x-8 gap-y-12 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {lots.map((lot) => (
            <article key={lot.title} className="group border border-[#e5e7eb] bg-white p-4 transition-all duration-300 hover:border-black hover:shadow-[0_20px_40px_-20px_rgba(0,0,0,0.1)]">
              <div className="relative mb-6 aspect-[3/4] overflow-hidden bg-[#f7f7f7]">
                <Image
                  src={lot.imageSrc}
                  alt={lot.imageAlt}
                  fill
                  sizes="(min-width: 1280px) 22vw, (min-width: 1024px) 30vw, (min-width: 768px) 44vw, 92vw"
                ...
                <div className="absolute top-4 left-4 flex items-center gap-2 border border-[#c4c7c7]/30 bg-white/90 px-3 py-1 backdrop-blur-md">
                  <span className={`h-1.5 w-1.5 rounded-full ${statusToneClass[lot.statusTone]} ${lot.statusTone === 'live' ? 'animate-pulse' : ''}`} />
                  <span className="text-[10px] tracking-[0.08em] text-black uppercase">{lot.status}</span>
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <h2 className="text-lg font-bold tracking-[-0.04em] text-black uppercase" style={headlineFont}>
                    {lot.title}
                  </h2>
                  <ShieldCheck className="h-5 w-5 shrink-0 text-[#747777] transition-colors group-hover:text-black" />
                </div>
                <div className="flex items-end justify-between gap-4 border-t border-[#c4c7c7]/30 pt-4">
                  <div>
                    <p className="mb-1 text-[10px] tracking-[0.25em] text-[#747777] uppercase">Current Bid</p>
                    <p className="text-lg font-bold text-black uppercase" style={headlineFont}>
                      {lot.bid}
                    </p>
                  </div>
                  <button type="button" className="bg-black px-4 py-2 text-[10px] font-bold tracking-[0.2em] text-white uppercase transition hover:bg-neutral-800">
                    Place Bid
                  </button>
                </div>
              </div>
        ...
        <section className="mt-24 flex flex-col items-center gap-8">
          <button type="button" className="group flex items-center gap-6 text-sm tracking-[0.3em] text-black uppercase transition-all" style={headlineFont}>
            <span className="h-px w-12 bg-black transition-all group-hover:w-24" />
            <span>Load More Archives</span>
            <span className="h-px w-12 bg-black transition-all group-hover:w-24" />
          </button>
          <p className="text-[10px] tracking-[0.25em] text-[#747777] uppercase">Displaying 8 of 142 Active Auctions</p>
        </section>
      </main>
        ...
        <div className="mx-auto flex max-w-[1600px] flex-col items-center justify-between gap-6 md:flex-row">
          ...
          <div className="flex flex-wrap items-center justify-center gap-8">
            {['Privacy', 'Terms', 'Provenance', 'Contact'].map((item) => (
              <Link key={item} href="#" className="text-xs tracking-[0.22em] text-[#747777] uppercase transition-colors hover:text-black">
                {item}
              </Link>
            ))}
          </div>
