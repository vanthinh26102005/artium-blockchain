...
import type { CreateEventRequest, EventApiResponse, EventLocation } from '@shared/apis/eventsApis'
...
export const mapApiEventToHostingEvent = (event: EventApiResponse): HostingEvent => {
  ...
  const endDateTime =
     ...
     * @returns void
     */
    event.endTime || new Date(new Date(startDateTime).getTime() + 60 * 60 * 1000).toISOString()
