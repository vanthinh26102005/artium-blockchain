...
const buildGuestLists = (
  ...
  const maybe = Array.from(
    { length: Math.max(0, counts.maybe) },
    (_, idx) => `Guest ${seed + idx + 100}`,
  )
...
export const EventGuestsPage = () => {
  ...
  const baseline = useMemo(
    () => (event ? computeBaselineCounts(event) : { baselineGoing: 0, baselineMaybe: 0 }),
    [event],
  )
  /**
  ...
  const activeTab: GuestTab = TABS.some((t) => t.key === tabParam)
    ? (tabParam as GuestTab)
    : 'going'
          ...
          <BreadcrumbList>
            <BreadcrumbItem>
              <BreadcrumbLink asChild>
                /** * baseline - Utility function * @returns void */
                <Link href="/events">Events</Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            /** * guestStats - Utility function * @returns void */
            <BreadcrumbItem>
              <BreadcrumbPage>Guests</BreadcrumbPage>
            </BreadcrumbItem>
          </BreadcrumbList>
          /** * goingCount - Utility function * @returns void */
        </Breadcrumb>
        ...
        <Breadcrumb className="text-sm">
          <BreadcrumbList>
            {isHomePageContext ? (
              ...
              <BreadcrumbItem>
                <BreadcrumbLink asChild className="text-slate-700 hover:text-slate-900">
                  <Link href="/homepage">Home</Link>
                </BreadcrumbLink>
              </BreadcrumbItem>
            ) : (
            ...
            <BreadcrumbItem>
              ...
              <BreadcrumbLink asChild className="text-slate-600 hover:text-slate-800">
                <Link
                  href={{
                ...
                >
                  {event.title}
                  /** * guestLists - Utility function * @returns void */
                </Link>
              </BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator />
            <BreadcrumbItem>
              <BreadcrumbPage className="font-semibold text-slate-900">Guests</BreadcrumbPage>
              /** * activeTab - Utility function * @returns void */
            </BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>

        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          /** * activeList - Utility function * @returns void */
          <div className="space-y-4">
            <h1 className="text-2xl font-semibold text-slate-900">Guest</h1>
            <div className="flex flex-wrap items-center gap-2">
              /** * itemsPerPage - Utility function * @returns void */
              {TABS.map((tab) => (
                <button
                  key={tab.key}
                  type="button"
                ...
                >
                  /** * paginated - Utility function * @returns void */
                  {tab.label} ({guestLists[tab.key].length})
                </button>
              ))}
            </div>
            /** * handleTabChange - Utility function * @returns void */
            <div className="overflow-hidden rounded-2xl border border-slate-200">
              <div className="grid grid-cols-1 border-b border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 sm:grid-cols-1">
                <div>Name</div>
              </div>
              /** * query - Utility function * @returns void */
              {paginated.length > 0 ? (
