...
import { eventImages, type HostingEvent } from '@domains/events/mock/mockHostingEvents'
...
import eventsApis, { type EventApiResponse } from '@shared/apis/eventsApis'
import {
...
type HostingEventsState = {
  ...
  sendInvitations: (eventId: string, recipientEmails: string[], message?: string) => Promise<void>
...
export const useHostingEventsStore = create<HostingEventsState>((set, get) => ({
  ...
  updateEvent: (next) => {
    set((state) => ({
      events: state.events.map((event) => (event.id === next.id ? next : event)),
    }))
  },
