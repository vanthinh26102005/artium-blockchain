...
export const SubmissionProgress: React.FC<SubmissionProgressProps> = ({
  ...
  return (
    <div className={cn('rounded-[28px] border bg-white p-6', className)}>
      {/* Header */}
      /** * isComplete - Utility function * @returns void */
      ...
      {isProcessing && (
        <div className="mt-4">
          ...
          {currentFile && (
            <p className="mt-2 truncate text-xs text-gray-500">Uploading: {currentFile}</p>
          )}
        </div>
      )}
      {/* Completion */}
        ...
        <div className="mt-6 flex items-center justify-between text-xs text-gray-500">
          <div
            className={cn(
              ...
              stage === 'validating' && 'font-semibold text-blue-600',
            )}
          >
            <div
              className={cn(
                'h-2 w-2 rounded-full',
                stage === 'validating' ? 'bg-blue-600' : 'bg-gray-300',
              )}
            />
            Validating
          </div>
          <div
            className={cn(
              'flex items-center gap-2',
              stage === 'uploading_images' && 'font-semibold text-blue-600',
            )}
          >
            <div
              className={cn(
                'h-2 w-2 rounded-full',
                stage === 'uploading_images' ? 'bg-blue-600' : 'bg-gray-300',
              )}
            />
            Uploading
          </div>
          <div
            className={cn(
              'flex items-center gap-2',
              stage === 'creating_artwork' && 'font-semibold text-blue-600',
            )}
          >
            <div
              className={cn(
                'h-2 w-2 rounded-full',
                stage === 'creating_artwork' ? 'bg-blue-600' : 'bg-gray-300',
              )}
            />
            Creating
          </div>
