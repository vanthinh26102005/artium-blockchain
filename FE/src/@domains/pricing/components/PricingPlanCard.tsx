...
export const PricingPlanCard: React.FC<PricingPlanCardProps> = ({ plan, billingCycle }) => {
  ...
  return (
    ...
    >
      {/* -- popular badge -- */}
      {plan.isPopular && (
        <div className="absolute right-0 top-0 -mt-3 mr-6 rounded-full bg-blue-600 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-white">
          Most Popular
        </div>
      )}
      ...
      <div className="mb-5 flex flex-col items-center text-center">
        <div className="flex items-end justify-center gap-2 text-slate-900">
          <span className="-translate-y-2 self-start text-sm font-semibold leading-none">
            {isFree ? '' : 'US$'}
          </span>
          <span className="text-6xl font-black leading-none">{isFree ? 'Free' : price}</span>
          {!isFree && (
            <span className="text-base font-semibold leading-none text-slate-800">/month</span>
          )}
        </div>
        {isFree ? (
          <div className="mb-1 mt-1 text-xs font-medium text-slate-800">Free Forever</div>
        ) : (
          <div className="mb-1 mt-1 text-xs font-medium text-slate-800">
      ...
      <div className="mt-2 flex flex-col items-center gap-2">
        ...
        {!isFree && billingCycle === BILLING_CYCLES.YEARLY && (
          <div className="mb-2 mt-2 inline-flex items-center gap-2 rounded-md border border-green-900 bg-[#A8FF8F] px-3 py-1 text-xs font-semibold text-green-800">
            <svg
