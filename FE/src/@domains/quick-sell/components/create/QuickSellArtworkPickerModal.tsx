...
export const QuickSellArtworkPickerModal = ({
    ...
    const loadArtworks = async () => {
      try {
        ...
        const mapped = response.map((artwork) => {
          const rawPrice =
            typeof artwork.price === 'string' ? parseFloat(artwork.price) : artwork.price
          const price =
  ...
  const filteredArtworks = useMemo(() => {
    ...
    return artworks.filter((artwork) => artwork.name.toLowerCase().includes(query))
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-[90vw] min-w-[600px] max-w-[900px] gap-0 overflow-hidden rounded-2xl bg-white p-0">
        {/* Header */}
        <div className="relative px-8 pb-4 pt-8">
          <h2 className="text-center text-[22px] font-bold text-[#191414]">Select Artwork</h2>
          /** * resetPicker - Utility function * @returns void */
          <button
            type="button"
            onClick={onClose}
            className="absolute right-6 top-6 p-2 text-[#989898] transition hover:text-[#191414]"
          >
            <X className="h-5 w-5" />
          </button>
          <p className="mt-3 text-center text-[13px] text-[#595959]">
            /** * handleDone - Utility function * @returns void */ Choose the artwork available from
            your inventory below.
            <br />
            You can only select single quantity artworks that are listed as For Sale or Inquire to
            Purchase.
          </p>
        </div>
        {/* Search */}
        <div className="px-8 pb-4">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#989898]" />
            <Input
              type="text"
              /**
              ...
              className="h-12 rounded-full border-[#E5E5E5] bg-white pl-12 pr-4 text-[14px] placeholder:text-[#989898] focus:border-[#191414] focus:ring-0"
            />
          </div>
        </div>
        /** * handleUploadArtwork - Utility function * @returns void */
        {/* Table */}
          ...
          <table className="w-full border-collapse">
            ...
            <thead className="sticky top-0 z-10">
              <tr className="bg-[#F5F5F5]">
                <th
                  className="rounded-tl-lg px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-[#989898]"
                  style={{ width: '40%' }}
                >
                  Artwork title
                </th>
                <th
                  className="px-3 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-[#989898]"
                  style={{ width: '12%' }}
                >
                  Year
                </th>
                ...
                <th className="rounded-tr-lg px-3 py-3" style={{ width: '8%' }}></th>
              </tr>
            </thead>
              ...
              {isLoading ? (
                ...
                filteredArtworks.map((artwork) => (
                  <tr
                    ...
                    className={`cursor-pointer transition hover:bg-[#FAFAFA] ${selectedArtworkId === artwork.id ? 'bg-blue-50' : ''}`}
                  >
                    {/* Artwork Title with Image */}
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[#F5F5F5]">
                          <img
                            ...
                            onError={(e) => {
                              ...
                              target.parentElement!.innerHTML =
                                '<span class="text-[8px] text-[#989898]">🖼</span>'
                            }}
                          />
                        </div>
                        <span className="max-w-[180px] truncate text-[13px] font-medium text-[#191414]">
                          {artwork.name}
                        </span>
                      </div>
                    </td>
                    ...
                    {/* Dimensions */}
                    <td className="whitespace-nowrap px-3 py-3 text-center text-[13px] text-[#595959]">
        ...
        <div className="border-t border-[#E5E5E5] px-8 py-6">
          ...
          <div className="flex justify-center gap-4">
            ...
            <Button
              ...
              className="h-12 min-w-[140px] rounded-full bg-blue-400 px-8 text-[14px] font-semibold text-white hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
            >
              Done
            </Button>
          </div>
