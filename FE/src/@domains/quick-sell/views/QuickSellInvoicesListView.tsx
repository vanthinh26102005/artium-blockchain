...
export const QuickSellInvoicesListView = () => {
  ...
  const handleSendInvoice = async (data: { name: string; email: string; message: string }) => {
    ...
    try {
       ...
       */
      setInvoices((prev) =>
        prev.map((inv) => (inv.invoiceCode === selectedInvoice.invoiceCode ? updatedInvoice : inv)),
      )
  ...
  return (
     ...
     * @returns void
     */
    <div className="-mx-6 -my-1 min-h-screen font-sans text-[#191414] sm:-mx-8 lg:-mx-12">
      {/* Header */}
      <div className="flex items-center justify-between gap-4 px-4 pb-4 pt-4 sm:px-6 lg:px-8">
        <div>
          <h1 className="text-3xl font-semibold leading-[120%] text-slate-900">Invoices</h1>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <Link href="/artist/invoices/create">
                  <Button
                    variant="outline"
                    className="rounded-full border-[#E5E5E5] px-6 font-bold text-[#191414]"
                  >
                    Create Invoice
                  </Button>
                </Link>
...
const InvoiceCard = ({
  ...
  return (
    <div className="group rounded-3xl border border-black/5 bg-white p-6 shadow-sm transition hover:border-black/10">
      {/* Top Row */}
      <div className="mb-8 flex items-start justify-between">
        <div className="flex gap-4">
          {/* Thumbnail */}
          <div className="h-[60px] w-[60px] overflow-hidden rounded-xl border border-[#E5E5E5] bg-[#F5F5F5]">
            {artwork?.imageUrl ? (
          ...
          {/* ID */}
          <div>
            <h3 className="text-[16px] font-bold text-[#191414] transition-colors group-hover:text-blue-600">
              /** * InvoiceCard - React component * @returns React element */ Invoice #
              {invoice.invoiceCode}
            </h3>
            <p className="mt-1 text-[13px] text-[#595959]">Artist</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          >
            ...
            /** * isPaid - Utility function * @returns void */ Send Invoice
          </Button>
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
        ...
        <div className="space-y-4">
          <div className="flex justify-between md:block md:space-y-1">
            ...
            <p className="max-w-[200px] truncate text-[14px] font-bold text-[#191414]">
          ...
          <div className="flex justify-between md:block md:space-y-1">
            ...
            <p className="text-[14px] font-bold text-[#191414]">{formatMoney(invoice.subtotal)}</p>
            /** * url - Utility function * @returns void */
          </div>
          <div className="flex justify-between md:block md:space-y-1">
            <p className="text-[11px] font-bold uppercase tracking-wider text-[#989898]">
              Order Status
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-[14px] font-bold capitalize text-[#191414]">
        ...
        <div className="space-y-4">
          <div className="flex justify-between md:block md:space-y-1">
            ...
            <p className="text-[14px] font-bold text-[#191414]">Jan 23, 2026</p>
          </div>
          <div className="flex justify-between md:block md:space-y-1">
            <p className="text-[11px] font-bold uppercase tracking-wider text-[#989898]">Sent to</p>
            <p className="text-[14px] font-bold text-[#191414]">{invoice.buyer?.email || '-'}</p>
          </div>
          <div className="flex justify-between md:block md:space-y-1">
            <p className="text-[11px] font-bold uppercase tracking-wider text-[#989898]">
              Last sent
            </p>
            <p className="text-[14px] font-bold text-[#191414]">-</p>
          </div>
        </div>
      </div>
    </div>
