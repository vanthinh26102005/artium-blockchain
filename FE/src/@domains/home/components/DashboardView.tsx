    ...
    >
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500">
          <img src={avatarUrl} alt={author} className="h-4 w-4 rounded-full object-cover" />
          <span>{author}</span>
          <span>•</span>
          <span>{date}</span>
          <span>•</span>
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center">
          <span className="inline-flex items-center gap-1 rounded-full border border-slate-200 px-2 py-0.5 text-xs font-semibold text-slate-900">
            <span
              className={`h-1.5 w-1.5 rounded-full ${item.isSold ? 'bg-slate-400' : 'bg-blue-600'}`}
            ></span>
            {priceLabel}
          </span>
        </div>
...
export const DashboardView = () => {
  ...
  const events = useEventsStore(
    useShallow((state) => state.allEvents.filter((e) => e.id.startsWith('home-ev-'))),
  )
 ...
 */
      ...
      <div className="space-y-12">
        {/* Weekly Editorial */}
        <section>
          /** * navigateToSection - Utility function * @returns void */
          <div
        ...
        <section className="overflow-visible">
          ...
          >
            ...
            <ChevronRight className="h-6 w-6 text-slate-400 transition-colors group-hover:text-slate-900" />
            /** * handleInvite - Utility function * @returns void */
          </div>
          <div className="no-scrollbar -mb-[180px] -mr-6 flex gap-6 overflow-x-auto overflow-y-visible pb-[200px] pr-6">
            {events.map((event) => (
