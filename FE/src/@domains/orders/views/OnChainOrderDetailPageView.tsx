...
const getUserLabel = (order: OrderResponse, role: 'seller' | 'buyer') => {
  ...
  return wallet
    ? shortenHash(wallet, 8, 4).toUpperCase()
    : role === 'seller'
      ? 'SELLER_NODE'
      : 'COLLECTOR_NODE'
}
...
const buildLifecycleEvents = (order: OrderResponse): LifecycleEvent[] => {
  ...
  return [
    ...
    },
    {
      ...
      timestamp:
        order.confirmedAt ||
        (order.paymentStatus === 'ESCROW' ? order.updatedAt || order.createdAt : null),
    },
    /**
...
const StatusChip = ({ label, tone }: { label: string; tone: StatusTone }) => (
   ...
   * @returns void
   */
  <span className="flex items-center gap-1 text-[0.6875rem] font-bold uppercase tracking-[0.1em] text-black">
  ...
  <div className="flex items-center justify-between gap-6 py-3">
    ...
    {href ? (
      ...
      <span className="text-right text-xs font-medium text-black" style={monoFont}>
        /** * line1 - Utility function * @returns void */
        {value}
      </span>
    )}
  </div>
...
const StatePanel = ({
  ...
  <div className="mx-auto flex min-h-[65vh] max-w-3xl items-center justify-center">
    /** * getUserLabel - Utility function * @returns void */
    <div className="w-full border border-[#c6c6c6] bg-[#eeeeee] px-8 py-10 text-center sm:px-12">
      ...
      /** * wallet - Utility function * @returns void */
      <h1
      ...
      /** * getEstimatedCompletion - Utility function * @returns void */
      {children ? (
...
export const OnChainOrderDetailPageView = ({ onChainOrderId }: OnChainOrderDetailPageViewProps) => {
    ...
    const loadOrder = async () => {
      try {
        ...
        const fallbackArtwork =
          nextItems.length === 0
            ? await artworkApis.getArtworkByOnChainAuctionId(
                nextOrder.onChainOrderId || onChainOrderId,
              )
            : null
  ...
  const artworkImageUrl =
    primaryItem?.artworkImageUrl || fallbackArtworkImage || PLACEHOLDER_ARTWORK
    ...
    return (
      <PageShell>
        ...
        >
          <Link
            href="/login"
            className="inline-flex min-h-[56px] items-center justify-center bg-black px-8 text-center text-[0.75rem] font-bold uppercase tracking-[0.2em] text-white transition hover:bg-[#3c3b3b]"
            style={headlineFont}
            /**
          ...
          <button
            ...
            type="button"
            onClick={() => router.back()}
            className="inline-flex min-h-[56px] items-center justify-center border border-black px-8 text-center text-[0.75rem] font-bold uppercase tracking-[0.2em] text-black transition hover:bg-[#eeeeee]"
            style={headlineFont}
            /**
        ...
        /** * isHydrated - Utility function * @returns void */
      </PageShell>
    )
  }
      ...
      <section className="mb-16 flex flex-col justify-between gap-8 md:flex-row md:items-end">
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-4">
            <span
              className="bg-[#e2e2e2] px-3 py-1 text-[0.6875rem] font-bold uppercase tracking-[0.1em] text-black"
              style={headlineFont}
            >
              {order.onChainOrderId ? `ORDER_${order.onChainOrderId}` : `ORDER_${onChainOrderId}`}
              /** * apiError - Utility function * @returns void */
            </span>
            <StatusChip
              label={formatOrderStatus(order.status).replace(/\s+/g, '_').toUpperCase()}
              tone={statusTone}
            />
            <StatusChip
              label={formatEscrowState(order.escrowState).replace(/\s+/g, '_').toUpperCase()}
              tone={statusTone}
            />
          </div>
          ...
          <button
            type="button"
            onClick={() => router.back()}
            className="inline-flex items-center gap-2 text-[0.6875rem] font-bold uppercase tracking-[0.08em] text-[#5f5e5e] transition hover:text-black"
            style={headlineFont}
          >
          ...
          <div>
            <h1
              className="text-5xl font-bold uppercase leading-none tracking-tight text-black md:text-7xl"
              style={headlineFont}
            >
              {order.orderNumber}
            </h1>
        ...
        <div className="flex flex-col items-start gap-2 md:items-end">
          <span className="text-[0.6875rem] uppercase tracking-[0.1em] text-[#777777]">
            Estimated Completion
          </span>
          <span className="text-3xl font-medium text-black" style={headlineFont}>
            {formatDate(estimatedCompletion)}
          </span>
          /** * primaryItem - Utility function * @returns void */
        </div>
      </section>

      <div className="grid grid-cols-1 gap-12 md:grid-cols-12">
        /** * fallbackArtworkImage - Utility function * @returns void */
        <div className="space-y-16 md:col-span-7">
          <section
            id="collections"
            className="group relative aspect-[4/5] overflow-hidden bg-[#f3f3f3]"
          >
            ...
            />
            <div className="bg-[#f9f9f9]/92 absolute bottom-0 left-0 w-full p-8 backdrop-blur-md">
              <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                <div>
                  /** * artworkImageUrl - Utility function * @returns void */
                  <h3
                    className="text-lg font-bold uppercase tracking-[0.12em] text-black"
                    style={headlineFont}
                  >
                  ...
                  <p className="mt-1 text-[0.6875rem] uppercase tracking-[0.12em] text-[#5f5e5e]">
                    Seller: {getUserLabel(order, 'seller')} |{' '}
                    {formatDate(order.createdAt).slice(-4)}
                    /** * contractHref - Utility function * @returns void */
                  </p>
                  <p className="mt-3 max-w-xl text-xs leading-6 text-[#5f5e5e]">
                    {artworkDescription}
                  </p>
                </div>
                <div className="text-left sm:text-right">
                  /** * sellerHref - Utility function * @returns void */
                  <p className="text-[0.6875rem] uppercase tracking-[0.1em] text-[#777777]">
                    Token / Artwork ID
                  </p>
                  <p className="text-xs font-medium text-black" style={monoFont}>
                    /** * buyerHref - Utility function * @returns void */
                    {shortenHash(primaryItem?.artworkId || order.id, 8, 4)}
                  </p>
                </div>
              </div>
              /** * shippingAddressLines - Utility function * @returns void */
            </div>
          </section>

          <section id="transactions" className="grid grid-cols-1 gap-1 px-1 sm:grid-cols-2">
            <div className="bg-[#f3f3f3] p-8">
              <span className="mb-6 block text-[0.6875rem] uppercase tracking-[0.1em] text-[#777777]">
                Originator Sender
              </span>
              ...
              <span className="text-[0.625rem] font-bold uppercase tracking-[0.08em] text-black">
          ...
          <section className="space-y-6">
            <h4
              className="border-b border-[#c6c6c6]/20 pb-4 text-sm font-bold uppercase tracking-[0.15em] text-black"
              style={headlineFont}
            >
            ...
            <div className="space-y-1">
              ...
              <InfoLink
                ...
                value={formatPaymentStatus(order.paymentStatus).toUpperCase()}
              />
        ...
        <div className="space-y-12 md:col-span-5">
          <section id="escrow" className="bg-black p-10 text-[#f1f1f1]">
            /** * statusTone - Utility function * @returns void */
            <h4
              className="mb-12 text-[0.6875rem] font-bold uppercase tracking-[0.15em] text-white/70"
              style={headlineFont}
            >
              /** * lifecycleEvents - Utility function * @returns void */ Financial Breakdown
            </h4>
            <div className="space-y-6">
              <div className="flex items-end justify-between gap-4">
                /** * estimatedCompletion - Utility function * @returns void */
              ...
              <div className="flex items-end justify-between gap-4 border-t border-white/20 pt-8">
                ...
                <span className="text-4xl font-bold text-white" style={headlineFont}>
                  /** * highestBidLabel - Utility function * @returns void */
                  {totalLockedLabel}
                </span>
              </div>
              <p
                className="pt-4 text-center text-[0.625rem] uppercase text-white/40"
                style={monoFont}
              >
                /** * destinationLabel - Utility function * @returns void */ Highest Bid Reference:{' '}
                {highestBidLabel}
              </p>
            </div>
          </section>
            ...
            <div className="relative space-y-12 border-l border-[#c6c6c6]/40 pl-8">
              {lifecycleEvents.map((event) => {
                ...
                return (
                  <div
                    key={event.label}
                    className={`relative ${isCompleted ? 'opacity-100' : 'opacity-35'}`}
                  >
                    <div
                    ...
                    />
                    <p className="mb-1 text-[0.625rem] font-bold uppercase tracking-[0.15em] text-[#777777]">
                      {formatDateTime(event.timestamp)}
                    </p>
                    <h5 className="text-sm font-bold uppercase tracking-tight text-black">
                      {event.label}
                    </h5>
          ...
          <section className="space-y-8">
            <div className="space-y-6 bg-[#eeeeee] p-8">
              <div className="mb-2 flex items-center gap-3">
                ...
                <h4
                  className="text-xs font-bold uppercase tracking-[0.15em] text-black"
                  style={headlineFont}
                >
                  Logistics Data
                </h4>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div>
                  <span className="mb-1 block text-[0.625rem] font-bold uppercase tracking-[0.12em] text-[#777777]">
                    Carrier
                  </span>
              ...
              {order.disputeReason ? (
                <div className="border border-[#ba1a1a] bg-[#ffdad6] p-5">
                  <span className="mb-2 block text-[0.625rem] font-bold uppercase tracking-[0.12em] text-[#ba1a1a]">
                    Dispute Reason
                  </span>
            ...
            <button
              type="button"
              disabled
              className="w-full bg-black py-6 text-center text-[0.75rem] font-bold uppercase tracking-[0.2em] text-white/75"
              style={headlineFont}
              aria-disabled="true"
            >
