...
export const PlanUpgradeModal = ({ isOpen, onOpenChange }: PlanUpgradeModalProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      /** * PlanUpgradeModal - React component * @returns React element */
      <DialogContent
        size="5xl"
        className="!max-h-[90vh] w-[95vw] overflow-y-auto rounded-3xl bg-white p-6 sm:p-10"
      >
          ...
          {plans.map((plan) => (
            ...
            >
              ...
              <div className="mb-4 text-center">
                ...
                <p className="mt-2 h-10 text-sm leading-snug text-slate-500">{plan.description}</p>
              </div>
              ...
              <button
                className={`w-full cursor-pointer rounded-full py-2.5 text-sm font-semibold transition-colors ${
                  plan.name === 'Basic'
                    ? 'pointer-events-none cursor-default bg-transparent text-slate-900'
              ...
              {plan.name !== 'Basic' && (
                <button className="mt-3 w-full cursor-pointer text-xs font-medium text-blue-600 hover:underline">
                  Start Free Trial
                </button>
              )}
            </div>
