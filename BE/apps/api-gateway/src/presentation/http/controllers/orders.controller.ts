...
export class OrdersController {
  ...
  async getOrderByOnChainId(
    @Param('onChainOrderId') onChainOrderId: string,
    @Req() req: any,
  ) {
    ...
    return this.getAuthorizedOrder(
      order.id,
      req.user?.id,
  ...
  @ApiResponse({
    status: 200,
    description: 'Order invoice retrieved',
    type: OrderInvoiceObject,
  })
  @ApiResponse({ status: 404, description: 'Order not found' })
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Open a dispute (buyer action, within 14 days of shipment)',
  })
  @ApiParam({ name: 'id', type: 'string', description: 'Order ID' })
  ...
  @ApiResponse({
    status: 400,
    description: 'Invalid status transition or dispute window expired',
  })
  @ApiResponse({ status: 404, description: 'Order not found' })
  async openDispute(
    @Param('id') id: string,
    @Body() data: OpenDisputeDto,
    @Req() req: any,
  ) {
    return sendRpc(
      this.ordersClient,
